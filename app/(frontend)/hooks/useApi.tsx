...
import { REACT_QUERY_RETRY, REACT_QUERY_STALE_TIME, REFECH_ON_WINDOW_FOCUS } from "config/global.config"
...
const queryFn = <ApiPayload, ApiResponse>(requestData: RequestData<ApiPayload>) => {
  ...
  if (requestData?.isExternalApi !== undefined) args.isExternalApi = requestData?.isExternalApi
  if (requestData?.setToken !== undefined) args.setToken = requestData?.setToken
...
export function useGenericQuery<ApiPayload, ApiResponse>({
  ...
  queryKey: string
  queryOptions?: Omit<UseQueryOptions<ApiResponse, unknown>, "queryKey" | "queryFn">
}) {
  return useQuery({
...
export function useGenericMutation<ApiPayload, ApiResponse>() {
  return useMutation({
    mutationFn: (requestData: RequestData<ApiPayload>) => queryFn<ApiPayload, ApiResponse>(requestData),
    onError: (error) => {
